...
CREATE TEMPORARY TABLE
	dinosaur4
SELECT
	f.title, i.film_id, s.store_id, r.rental_id, r.return_date, p.payment_date, i.inventory_id
FROM
	film f
JOIN
	inventory i
...
SELECT 
	title, return_date, payment_date, inventory_id 
FROM
	dinosaur4
WHERE
...
## YES. All four copies are available predicated upon the fact that the last return date for each copy comes after the last payment date.
